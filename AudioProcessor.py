...
def audio_generate_sine_wave(audio_data: Array, freq: float, amp: float, samples_per_sec: int) -> None:
    ...
    for ith_sample in audio_data:
        
        time = float(ith_sample/samples_per_sec)#time equals divide i by samples_per_sec
        theta = float(time * freq * 2*pi) #theta is equal to t * freq * 2pi
        audio_data[ith_sample] = float(sin(theta) * amp) #sets i-th sample audio_data[i] to sin(theta)*amp
    return audio_data
